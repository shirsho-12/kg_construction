...
def run_schema_refinement(config: Dict[str, Any], logger):
    ...
    if variant == "triplets_only":
        refiner.refine_schema_triplets_only(
            triplets_file=output_dir / refiner_config.get("triplets_file", "triplets.json"),
        ...
        refiner.refine_schema_triplets_synonyms_text(
            triplets_file=output_dir / refiner_config.get("triplets_file", "triplets.json"),
            synonyms_file=output_dir / refiner_config.get("synonyms_file", "synonyms.json"),
            input_file=output_dir / refiner_config.get("input_file", "results.json"),
            output_dir=output_dir / "schema_refinement",
        )
...
def run_qa_evaluation(config: Dict[str, Any], logger):
    ...
    qa_system.load_knowledge_graph_from_file(
        Path(qa_config.get("triplets_file"))
    )
...
def create_example_configs():
    ...
    text_config = {
        "pipeline": {
            "data_path": "data/text/example.txt",
            "output_dir": "output/text/example",
            "use_synonyms": True,
    ...
    json_config = {
        "pipeline": {
            ...
            "output_dir": "output/json/example",
            "use_synonyms": True,
            "compression_method": "agglomerative",
            "compression_threshold": 0.8,
            "compress_if_more_than": 30,
    ...
    qa_config = {
        "qa_evaluation": {
            "data_path": "data/json/test.json",
            "triplets_file": "output/json/example/triplets.json",
            "output_dir": "output/json/example"
        },
        "logging": {
            "level": "INFO"
...
Examples:
  ...
  python src/main.py --config configs/qa_evaluation.yaml --mode qa_evaluation
    ...
    parser.add_argument(
        "--mode",
        choices=["text", "json", "schema_refiner", "qa_evaluation"],
    ...
    if not args.config or not args.mode:
        parser.error("Both --config and --mode are required unless using --create-configs")
    ...
    try:
        ...
        elif args.mode == "schema_refiner":
            run_schema_refinement(config, logger)
        elif args.mode == "qa_evaluation":
