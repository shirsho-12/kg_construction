...
class ConfigManager:
    ...
    def load_config(self, specific_config_path: Path) -> Dict[str, Any]:
        ...
        specific_config = self._resolve_paths(specific_config, specific_config_path.parent.parent)
        ...
        if isinstance(config, dict):
            for key, value in config.items():
                if isinstance(value, dict):
                    ...
                    "data_path" in key or
                    "output_dir" in key or
                    key in ["cache_dir", "base_encoder_model"]
                ):
    ...
    def get_config_value(self, key_path: str, config: Optional[Dict[str, Any]] = None) -> Any:
        """
